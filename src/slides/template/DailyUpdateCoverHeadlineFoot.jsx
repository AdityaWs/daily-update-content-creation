...
export default function DailyUpdateCoverHeadlineFoot({ image, judul, materi, img_object_fit = 'cover', img_object_position = 'top', index = 1 }) {

  let image_to_display = null;
  if (image) {
    image_to_display = <img
      className="absolute top-[10%] left-0 h-[50%] w-full"
      style={{
  ...
  return (
    <article className="relative h-[720px] w-[540px] bg-white">
      ...
      <div className="absolute top-0 left-0 h-[30%] w-full bg-linear-to-b from-red-950/50 to-red-950/0"></div>
      <div className="absolute top-0 left-0 h-[10%] w-full bg-red-950"></div>
      <img src={logo} className="absolute top-0 left-0 h-[10%] ml-4 mt-4" />
      <div className="absolute bottom-0 left-0 h-[43%] p-8 pb-16 bg-red-900">
