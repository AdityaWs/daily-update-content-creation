...
export default function DailyUpdateCoverHeadline({ image, judul, materi, img_object_fit = 'cover', img_object_position = 'top', index = 1 }) {
  ...
  return (
    <article className="relative h-[720px] w-[540px] bg-white">
      {image_to_display}
      <div className="absolute bottom-0 left-0 h-[80%] w-full bg-linear-to-t from-red-950/50 to-red-900/0 opacity-50"></div>
      <div className="absolute top-[40%] left-0 h-[30%] w-full bg-linear-to-b from-zinc-950/20 to-zinc-950/0"></div>
      <div className="absolute top-0 left-0 h-[10%] w-full bg-red-900"></div>
      <img src={logo} className="absolute top-0 left-0 h-[10%] ml-4 mt-4" />
      <div className="absolute top-[10%] left-0 h-[30%] p-8 pt-0 bg-red-900">
        <h1 className="text-4xl mb-4 text-white font-bold drop-shadow">{judul}</h1>
        <p className="absolute bottom-0 left-0 translate-y-full text-justify text-white font-bold bg-red-950 px-4 py-2">{materi}</p>
      </div>
